...
> **⚠️ 升级提示**：这次更新需要管理员权限才能安装。如果应用内点「立即安装」装不上，请任选一种方式：
> 1. **右键 AgentPack →「以管理员身份运行」**，打开后再次点「立即安装」；
> 2. 到 **GitHub Releases 页面**手动下载并安装最新安装包
...
- **Windows 应用内无法启动更新安装器（提权）**：0.2.3 改用 CreateProcess 启动后无法在普通权限下触发 UAC 提权（`ERROR_ELEVATION_REQUIRED`），改为 `ShellExecuteW("runas")` 提权启动，恢复 0.2.2 经 `cmd start` 的自动提权能力，同时不经 cmd.exe 避免元字符命令注入
- **NSIS 「记住上次安装位置」在 machine 级安装器下失效（提权）**：提权后写 HKCU 落到管理员账户、普通用户读取不到，改用 HKLM 全用户共享，普通权限应用升级时也能读到上次安装目录
