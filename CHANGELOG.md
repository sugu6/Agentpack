...
- Skills 更新检测增强：查询失败不覆盖本地缓存基线，避免错误状态污染后续检查；空分支先查 `main`，失败自动回退 `master`；同一仓库多个 skill 共享错误信息，保留失败原因映射

### 变更

- Agents 页面标题改为 "Agents"，Agent 卡片按变体独立拆分显示，启用/禁用改用 Switch 开关，删除 MCP 数量卡片；统一 Agent 管理入口到 Agents 页面，删除设置页 Agents 管理卡片
- Skills 页面标题与描述布局调整：英文长描述与标题同行显示，按钮独立第二行右对齐，避免描述挤占按钮空间
- Skills 更新按钮优化：检测到更新时"全部更新"按钮替换"检查更新"按钮，避免并排遮挡；单个 skill 更新图标改为向上箭头，更直观表达升级含义
- Skills 更新 toast 语义调整：全部为最新时显示绿色成功提示，发现更新时显示蓝色信息提示

### 修复

- MCP 页面标题改为 i18n 翻译，不再硬编码 "MCP 服务器"
